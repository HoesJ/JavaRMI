...
public abstract class AbstractTestBooking extends AbstractTesting {
	 ...
	 * @throws Exception if things go wrong, throw exception
	 */
	protected abstract Quote createQuote(String clientName, Date start, Date end, String carType, String region)
			throws Exception;
	 ...
	 *
	 * @param renter name of the client
	...
	/**
	 * 
	 * This class supports the following script commands (note that additional
	 * commands can be supported by extending this class) :
	 * 
	 * BA <from> <until> Check availability of car types from <from> until <until>
	 * 
	 * BB <from> <until> <carType> (command) Create a quote for <client> for a car
	 * of type <carType> from <from> until <until>
	 * 
	 * BF Finalize the quote made for client <client>
	 * 
	 * BM <type:nr>* Assess total number of reservations comparing it to <type:nr>*
	 * 
	 * BMR Print all reservations of a client
	 *
	 * c Modifier to indicate that the according command will fail
	 */
	final protected void processLine(String name, String cmd, List<Character> flags, StringTokenizer scriptLineTokens)
		...
		if (cmd.equals("BA")) {
			...
			try {
				assessTotalReservationsRenter(name, scriptLineTokens);
			} catch (Exception e) {
				throw new ApplicationException(e);
			}
		}
