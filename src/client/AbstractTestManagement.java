 ...
 */
public abstract class AbstractTestManagement<ReservationSession, ManagerSession> extends AbstractTestBooking<ReservationSession, ManagerSession> {
     ...
     * @throws Exception if things go wrong, throw exception
     */
    protected abstract CarType getMostPopularCarTypeInCRC(ManagerSession ms, String carRentalCompanyName, int year) throws Exception;
	...
	private void checkPopularCarType(String name, StringTokenizer scriptReader) throws Exception {
       ...
       while (scriptReader.hasMoreTokens()) {
           ...
           CarType ct = getMostPopularCarTypeInCRC(rental, name, year);
           if (ct != null) {
               favorite = ct.getName();
